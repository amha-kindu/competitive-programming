class Solution:
    def friendRequests(self, n: int, restrictions: List[List[int]], requests: List[List[int]]) -> List[bool]:
        ...
        for p1, p2 in requests:
            possible = True
            p1_friends = set(friends.components(p1))
            p2_friends = set(friends.components(p2))
            for src, tgt in restrictions:
                if src in p1_friends and tgt in p2_friends or\
                    src in p2_friends and tgt in p1_friends:
                    possible = False
                    break
            if possible:
                friends.union(p1, p2)
